...
impl App {
    ...
    pub fn get_current_token_id(&self) -> Option<String> {
        // If we're in event market selector mode, get token from event market
        if self.show_event_market_selector || (!self.show_market_selector && !self.show_token_selector) {
    ...
    pub fn select_event(&mut self) {
        if !self.filtered_events.is_empty() && self.selected_event < self.filtered_events.len() {
            ...
            // Check if the event has markets
            if let Some(ref markets) = event.markets {
                if !markets.is_empty() {
                    self.selected_market = 0; // Reset to first market in the event
                    self.selected_token = 0;
                    self.token_scroll_offset = 0;
                    self.show_market_selector = false;
                    self.show_event_market_selector = true; // Show markets within the event
                    self.search_mode = false;
                    self.needs_redraw = true;
                }
            }
        ...
        if !self.filtered_events.is_empty() && self.selected_event < self.filtered_events.len() {
            ...
            if let Some(ref markets) = event.markets {
                if self.selected_market < markets.len() {
                    ...
                    // For gamma markets, we need to extract token IDs from clob_token_ids
